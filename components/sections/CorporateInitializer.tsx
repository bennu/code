    ...
    <Box
      ...
      onMouseEnter={() => {
        if (!disabled && cardRef.current)
          animate(cardRef.current, { scale: 1.025, duration: 180, ease: "easeOutSine" })
      }}
      onMouseLeave={() => {
        if (!disabled && cardRef.current)
          animate(cardRef.current, { scale: 1, duration: 180, ease: "easeOutSine" })
      }}
      sx={{
        background: disabled
          ? dark
    ...
    >
      {disabled && (
        <Box
          sx={{
            ...
            px: 1,
            py: 0.25,
            background: dark ? "#1c1f3a" : "#f1f5f9",
            border: dark ? "1px solid #252a45" : "1px solid #cbd5e1",
            borderRadius: "4px",
            fontFamily: MONO,
            fontSize: "0.62rem",
            color: MUTED,
      ...
      {!disabled && (
        ...
        >
          {selected && (
            <Box
              component="span"
              sx={{ color: dark ? "#000" : "#fff", fontSize: "0.6rem", lineHeight: 1 }}
            >
              ✓
            </Box>
          )}
        ...
        {template.logo.startsWith("http") ? (
          <Box
            ...
            sx={{
              ...
              height: 26,
              objectFit: "contain",
              filter: disabled || selected ? "none" : "grayscale(30%) opacity(0.6)",
            }}
          />
        ) : (
      ...
      <Box
        sx={{
          ...
          px: 1,
          py: 0.2,
          background: selected ? `${ACCENT}1a` : dark ? "#1c1f3a" : "#f1f5f9",
          borderRadius: "4px",
          fontFamily: MONO,
          fontSize: "0.68rem",
  ...
  return (
    <Box sx={{ mb: 2 }}>
      ...
      >
        {loading ? (
          <Box
            sx={{ color: "#64748b", fontFamily: MONO, fontSize: "0.73rem", fontStyle: "italic" }}
          >
            cargando…
          </Box>
        ) : lines.length === 0 ? (
  ...
  return (
    <Box
      ...
      sx={{
        ...
        py: 1.75,
        cursor: "pointer",
        background: selected ? ACCENT_LIGHT : dark ? "#1a1d30" : "#fff",
        transition: "all 0.18s",
        textAlign: "center",
        userSelect: "none",
    ...
    >
      ...
      <Box sx={{ fontFamily: SANS, fontWeight: 600, fontSize: "0.83rem", color: SLATE }}>
        {label}
      </Box>
...
export default function CorporateInitializer() {
  ...
  useEffect(() => {
    ...
    if (selectedBackend && backendTreeLines.length === 0)
      fetches.push(
        fetchTreePaths(selectedBackend).then((p) => setBackendTreeLines(renderTree(buildTree(fixPkgOrder(p, groupId))))),
      )
    if (selectedFrontend && frontendTreeLines.length === 0)
      fetches.push(
        fetchTreePaths(selectedFrontend).then((p) =>
          setFrontendTreeLines(renderTree(buildTree(fixPkgOrder(p, groupId)))),
        ),
      )
    if (fetches.length === 0) {
  ...
  useEffect(() => {
    ...
    const pi = setInterval(
      () => setDlProgress((p) => (p >= 85 ? p : p + Math.random() * 3.5 + 0.8)),
      130,
    )
    return () => {
      clearInterval(si)
              ...
              <Box>
                ...
                <Box sx={{ display: "flex", gap: 1.5 }}>
                  ...
                  <TypeCard
                    ...
                    desc="Servidor"
                    icon="⚙️"
                    selected={projectType === "backend"}
                    onSelect={() => setProjectType("backend")}
                  />
                  <TypeCard
                    value="frontend"
                    label="Solo Frontend"
                ...
                >
                  ...
                  <Box>
                    <Box
                      sx={{ fontFamily: SANS, fontWeight: 600, fontSize: "0.85rem", color: SLATE }}
                    >
                      Incluir{" "}
                      <Box component="span" sx={{ fontFamily: MONO, color: ACCENT }}>
                        docker-compose.yml
                      </Box>
                    </Box>
                ...
                {needsBackend && (
                  <Box sx={{ mb: needsFrontend ? 3.5 : 0 }}>
                    ...
                    >
                      ...
                      <Box
                        component="span"
                        sx={{ fontWeight: 400, color: MUTED, fontSize: "0.82rem" }}
                      >
                        (opcional)
                      </Box>
                    </Box>
                    ...
                    >
                      {BACKEND_TEMPLATES.map((t, i) => (
                        <TemplateCard
                          ...
                          onToggle={() =>
                            setSelectedBackendId((prev) => (prev === t.id ? null : t.id))
                          }
                          index={i}
                        />
                      ))}
                    ...
                    >
                      {FRONTEND_TEMPLATES.map((t, i) => (
                        <TemplateCard
                          ...
                          onToggle={() =>
                            setSelectedFrontendId((prev) => (prev === t.id ? null : t.id))
                          }
                          index={i}
                        />
                      ))}
              ...
              <Box>
                ...
                >
                  {[
                    ...
                    ...(isJavaTemplate(selectedBackend)
                    ...
                    <Box key={label} sx={{ display: "flex", alignItems: "baseline", gap: 2 }}>
                      <Box
                        sx={{ fontFamily: MONO, fontSize: "0.73rem", color: MUTED, minWidth: 72 }}
                      >
                        {label}:
                      </Box>
                      <Box
                ...
                {!canDownload && (
                  <Box
                    sx={{
                      ...
                      border: dark ? "1px solid #78350f" : "1px solid #fde68a",
                      borderRadius: "8px",
                      px: 2,
                      py: 1.25,
                      fontFamily: SANS,
