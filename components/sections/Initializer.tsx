...
function HistoryLine({ entry }: { entry: HistoryEntry }) {
  return (
    <Box sx={{
      ...
      '@keyframes tslIn': { from: { opacity: 0, transform: 'translateY(6px)' }, to: { opacity: 1, transform: 'translateY(0)' } },
      animation: 'tslIn 0.18s ease both',
    }}>
      {entry.type === 'command' && (
        <><Prompt /><Box component="span" sx={{ color: TEXT }}>&nbsp;bennu init</Box></>
      )}
      {entry.type === 'info' && (
        <Box component="span" sx={{ color: DIM }}>Initializing project wizard…</Box>
      )}
      {entry.type === 'success' && (
        <Box component="span">
          <Box component="span" sx={{ color: G }}>✔&nbsp;</Box>
          <Box component="span" sx={{ color: '#888' }}>{entry.label}</Box>
          <Box component="span" sx={{ color: '#555' }}>=</Box>
          <Box component="span" sx={{ color: TEXT }}>"{entry.value}"</Box>
        </Box>
      )}
    </Box>
  );
...
function TemplateList({
  templates, selectedId, onToggle,
}: {
  templates: TemplateItem[]; selectedId: string | null; onToggle: (id: string) => void;
}) {
  return (
    <Box sx={{ mt: 0.5 }}>
      {templates.map((t, i) => {
        ...
        return (
          <Box
            ...
            sx={{
              ...
              py: 0.25, px: 1, cursor: disabled ? 'not-allowed' : 'pointer',
              ...
              userSelect: 'none',
              '@keyframes tslIn': { from: { opacity: 0, transform: 'translateX(-6px)' }, to: { opacity: 1, transform: 'translateX(0)' } },
          ...
          >
            ...
            <Box component="span" sx={{ color: '#444', fontSize: '0.78rem' }}>
              # {t.badge}{disabled ? ' (próximamente)' : ''}
            </Box>
          </Box>
        );
      })}
      <Box sx={{ py: 0.25, px: 1, pl: '27px', color: DIM, fontSize: '0.8rem', fontStyle: 'italic', userSelect: 'none' }}>
        (press Enter to skip)
      </Box>
    </Box>
  );
...
function TreeBlock({ label, lines, loading }: { label: string; lines: string[]; loading: boolean }) {
  return (
    <Box sx={{ mb: 1 }}>
      <Box sx={{ display: 'flex', gap: 1, mb: 0.25 }}>
        <Prompt />
        <Box component="span" sx={{ color: TEXT }}>&nbsp;tree {label}/</Box>
      </Box>
      <Box sx={{ pl: 2 }}>
        {loading ? (
          ...
          lines.map((line, i) => (
            <Box key={i} component="div" sx={{ whiteSpace: 'pre', fontFamily: MONO, fontSize: '0.8rem', color: '#bbb', lineHeight: 1.65 }}>
              {line}
            </Box>
          ))
        )}
...
function ProgressBar({ progress }: { progress: number }) {
  ...
  return (
    <Box component="span" sx={{ fontFamily: MONO }}>
      ...
      <Box component="span" sx={{ color: G }}>{'█'.repeat(filled)}</Box>
      <Box component="span" sx={{ color: '#2a2a2a' }}>{'░'.repeat(empty)}</Box>
      <Box component="span" sx={{ color: '#333' }}>]</Box>
      <Box component="span" sx={{ color: TEXT, ml: 1 }}>{Math.round(progress)}%</Box>
    </Box>
  );
}
...
export default function Initializer() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      handler = (e: KeyboardEvent) => { if (e.key === 'Enter') confirmPhase(); };
  ...
  useEffect(() => {
    ...
    if (selectedBackend)
      fetches.push(fetchTreePaths(selectedBackend).then(paths => setBackendTreeLines(renderTree(buildTree(paths)))));
    if (selectedFrontend)
      fetches.push(fetchTreePaths(selectedFrontend).then(paths => setFrontendTreeLines(renderTree(buildTree(paths)))));
  ...
  useEffect(() => {
    if (phase !== 8) return;
    const h = (e: KeyboardEvent) => { if (e.key === 'y' || e.key === 'Y') resetWizard(); };
    window.addEventListener('keydown', h);
    return () => window.removeEventListener('keydown', h);
  }, [phase, resetWizard]);
  ...
  const confirmPhase = useCallback(() => {
    if (enterBtnRef.current) {
      animate(enterBtnRef.current, { scale: [1, 0.9, 1], duration: 200, ease: spring({ bounce: 0.4 }) });
    }
    ...
    if (phase === 0) {
      ...
      const typeLabel = projectType === 'full' ? 'full (back+front)' : projectType === 'backend' ? 'solo backend' : 'solo frontend';
  ...
  const goBack = useCallback(() => {
    ...
    if (backBtnRef.current) {
      animate(backBtnRef.current, { scale: [1, 0.9, 1], duration: 200, ease: spring({ bounce: 0.4 }) });
    }
  ...
  return (
    ...
    >
      <Box sx={{ width: '100%', maxWidth: 700 }}>
        ...
        {/* ── title bar ── */}
        <Box sx={{
          background: '#252525', borderRadius: '10px 10px 0 0',
          border: '1px solid #333', borderBottom: '1px solid #1a1a1a',
          px: 2, py: 1.1, display: 'flex', alignItems: 'center', gap: 0.8,
        }}>
          <Box sx={{ width: 13, height: 13, borderRadius: '50%', background: '#ff5f57', flexShrink: 0 }} />
          <Box sx={{ width: 13, height: 13, borderRadius: '50%', background: '#febc2e', flexShrink: 0 }} />
          <Box sx={{ width: 13, height: 13, borderRadius: '50%', background: '#28c840', flexShrink: 0 }} />
          <Box sx={{ fontFamily: MONO, fontSize: '0.72rem', color: '#666', mx: 'auto', letterSpacing: '0.03em' }}>
        ...
        <Box
          ...
          sx={{
            ...
            px: '1.75rem', py: '1.5rem',
            minHeight: 460, maxHeight: 560, overflowY: 'auto',
            fontFamily: MONO, fontSize: '0.875rem', lineHeight: 1.85,
            cursor: 'text',
            '&::-webkit-scrollbar': { width: 4 },
            '&::-webkit-scrollbar-thumb': { background: '#2a2a2a', borderRadius: 2 },
          }}
        >
          {history.map(e => <HistoryLine key={e.id} entry={e} />)}
          <Box sx={{ height: '0.4rem' }} />

          {/* ── phase 0: project name ── */}
          {phase === 0 && (
            <Box sx={{ '@keyframes tslIn': { from: { opacity: 0 }, to: { opacity: 1 } }, animation: 'tslIn 0.2s ease both' }}>
              <Box sx={{ color: questionColor, mb: 0.25 }}>
                <Box component="span" sx={{ color: DIM }}>?</Box>&nbsp;project name:
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pl: 0.5 }}>
                ...
                <TextField
                  ...
                  onKeyDown={e => { if (e.key === 'Enter') { e.stopPropagation(); confirmPhase(); } }}
                  placeholder="my-awesome-app"
                  variant="standard" autoFocus
                  sx={{
                    flex: 1,
                    '& .MuiInput-root': { color: TEXT, fontFamily: MONO, fontSize: '0.875rem', '&:before, &:after': { display: 'none' } },
                    '& .MuiInput-input': { p: 0, caretColor: G, '&::placeholder': { color: '#3a3a3a', opacity: 1 } },
                  }}
                />
              </Box>
            </Box>
          ...
          {phase === 1 && (
            <Box sx={{ '@keyframes tslIn': { from: { opacity: 0 }, to: { opacity: 1 } }, animation: 'tslIn 0.2s ease both' }}>
              <Box sx={{ color: questionColor }}>
                <Box component="span" sx={{ color: DIM }}>?</Box>&nbsp;project type:
              </Box>
          ...
          {phase === 2 && (
            <Box sx={{ '@keyframes tslIn': { from: { opacity: 0 }, to: { opacity: 1 } }, animation: 'tslIn 0.2s ease both' }}>
              <Box sx={{ color: questionColor }}>
                <Box component="span" sx={{ color: DIM }}>?</Box>&nbsp;include docker-compose?
              </Box>
              <OptionList<string>
                options={[
          ...
          {phase === 3 && (
            <Box>
              <Box sx={{ color: questionColor }}>
                <Box component="span" sx={{ color: DIM }}>?</Box>&nbsp;select backend&nbsp;
                <Box component="span" sx={{ color: DIM, fontSize: '0.8rem' }}>(optional)</Box>
              </Box>
              <TemplateList
          ...
          {phase === 4 && (
            <Box sx={{ '@keyframes tslIn': { from: { opacity: 0 }, to: { opacity: 1 } }, animation: 'tslIn 0.2s ease both' }}>
              <Box sx={{ color: questionColor, mb: 0.25 }}>
                <Box component="span" sx={{ color: DIM }}>?</Box>&nbsp;group id:&nbsp;
                <Box component="span" sx={{ color: DIM, fontSize: '0.8rem' }}>(e.g. com.mycompany)</Box>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pl: 0.5 }}>
                <Box component="span" sx={{ color: G }}>❯</Box>
                <TextField
          ...
          {phase === 5 && (
            <Box>
              <Box sx={{ color: questionColor }}>
                <Box component="span" sx={{ color: DIM }}>?</Box>&nbsp;select frontend&nbsp;
                <Box component="span" sx={{ color: DIM, fontSize: '0.8rem' }}>(optional)</Box>
              </Box>
              <TemplateList
          ...
          {phase === 6 && (
            <Box sx={{ '@keyframes tslIn': { from: { opacity: 0 }, to: { opacity: 1 } }, animation: 'tslIn 0.25s ease both' }}>
              {/* cat project.yaml */}
              <Box sx={{ display: 'flex', gap: 1, mb: 0.5 }}>
                <Prompt />
                <Box component="span" sx={{ color: TEXT }}>&nbsp;cat project.yaml</Box>
              </Box>
              <Box sx={{ pl: 2, color: '#888', borderLeft: '1px solid #2a2a2a', ml: 0.5, mb: 2 }}>
                {[
                  ...
                  <Box key={k}>
                    <Box component="span" sx={{ color: '#666' }}>{k}: </Box>
                    <Box component="span" sx={{ color: v === 'none' ? DIM : TEXT }}>{v}</Box>
                  </Box>
                ))}
              </Box>
          ...
          {/* ── phase 7: downloading ── */}
          {phase === 7 && (
            <Box sx={{ '@keyframes tslIn': { from: { opacity: 0 }, to: { opacity: 1 } }, animation: 'tslIn 0.15s ease both' }}>
              <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
                <Prompt />
                <Box component="span" sx={{ color: TEXT }}>&nbsp;bennu download {dlLabel}</Box>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, pl: 0.5, mb: 0.75 }}>
                <Box component="span" sx={{ color: G, fontFamily: MONO, fontSize: '1rem', lineHeight: 1, display: 'inline-block', minWidth: '1ch' }}>
                  {dlStatus === 'done' ? '✔' : SPINNER_FRAMES[dlSpinnerIdx]}
                </Box>
                <Box component="span" sx={{ color: dlStatus === 'done' ? G : '#888', fontSize: '0.82rem' }}>
              ...
              <Box sx={{ pl: 0.5, color: '#444', fontSize: '0.75rem' }}>
                {dlStatus === 'done'
                  ? <Box component="span" sx={{ color: '#555' }}>saved → ~/{dlLabel}</Box>
                  : <Box component="span">
                      <Box component="span" sx={{ color: '#3a3a3a' }}>ETA </Box>
                      <Box component="span" sx={{ color: '#555' }}>
                        {dlProgress < 30 ? 'calculating…' : `~${Math.max(1, Math.round((100 - dlProgress) / 12))}s`}
                      </Box>
                    </Box>
                }
              </Box>
          ...
          {phase === 8 && (
            <Box sx={{ '@keyframes tslIn': { from: { opacity: 0 }, to: { opacity: 1 } }, animation: 'tslIn 0.3s ease both' }}>
              <Box sx={{ display: 'flex', gap: 1, mb: 0.25 }}>
                <Prompt />
                <Box component="span" sx={{ color: TEXT }}>&nbsp;bennu download {dlLabel}</Box>
              </Box>
              <Box sx={{ pl: 0.5, mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
                  <Box component="span" sx={{ color: G }}>✔</Box>
                  <Box component="span" sx={{ color: '#888', fontSize: '0.82rem' }}>Download complete</Box>
                </Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, mb: 0.5 }}>
                <Prompt />
                <Box component="span" sx={{ color: questionColor }}>&nbsp;? start a new project?</Box>
              </Box>
              <Box sx={{ display: 'flex', gap: 2, pl: 2 }}>
                <Box
                  onClick={resetWizard}
                  sx={{
                    color: G, border: '1px solid rgba(74,222,128,0.25)', borderRadius: '4px',
                    px: 2, py: 0.5, cursor: 'pointer', fontSize: '0.8rem', userSelect: 'none',
                    '&:hover': { background: 'rgba(74,222,128,0.08)', borderColor: 'rgba(74,222,128,0.45)' },
                ...
                <Box sx={{
                  color: DIM, border: '1px solid #2a2a2a', borderRadius: '4px',
                  px: 2, py: 0.5, fontSize: '0.8rem', userSelect: 'none', cursor: 'default',
                }}>
                  [n] no
                </Box>
              </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1.5, px: 0.5 }}>
          ...
          <Box
            ...
            sx={{
              ...
              '&:hover': { color: '#888' }, transition: 'color 0.15s',
            }}
          >
            [esc] back
          </Box>
          ...
          {canEnter && phase !== 2 && (
            <Box
              ...
              sx={{
                fontFamily: MONO, fontSize: '0.75rem', color: G,
                border: '1px solid rgba(74,222,128,0.25)', borderRadius: '4px',
                px: 2, py: 0.6, cursor: 'pointer', userSelect: 'none',
                '&:hover': { background: 'rgba(74,222,128,0.08)', borderColor: 'rgba(74,222,128,0.45)' },
                transition: 'all 0.15s',
              }}
            >
