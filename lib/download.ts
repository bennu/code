...
const TEXT_EXT = /\.(java|kt|xml|yml|yaml|properties|gradle|go|mod|sum|json|ts|tsx|js|jsx|html|css|scss|md|txt|sh|env|gitignore)$/;
...
async function fetchZip(template: TemplateItem): Promise<JSZip> {
  const res = await fetch(
    `/api/download-template?repo=${template.repo}&branch=${template.branch}`,
  );
  ...
  if (backend) {
    ...
    if (isJavaBackend) {
      ...
      files = await extractFiles(zip, {
        content: (text) =>
          oldPkg ? text.replaceAll(oldPkg, newPkg) : text,
        path: (p) =>
          oldPkg
            ? p.replaceAll(pkgToPath(oldPkg), pkgToPath(newPkg))
            : p,
      });
    } else {
      files = await extractFiles(zip);
